...
app.get('/psychologist', async (req, res) => {
  try {
    ...
    res.render('psychologist', { psychologistData: result.rows });
...
app.get('/Blogs',async (req,res)=>{
   ...
   try {
    ...
    // Render an EJS template and pass the data to it
    res.render('Blogs', { blogData: result.rows });
...
app.post('/session_set', async (req, res) => {
  ...
  //25-09-2023 start 
  var meetlink = generateGoogleMeetlink();
  //25-09-2023 end
  console.log({
    session_id,
    psy_name,
  ...
  app.get('/read_more/:blog_id', async (req, res) => {
    ...
    try {
      ...
      if (blogResult.rows.length === 0) {
        ...
        res.render('read_more', { blog, comments }); // Render the read_more.ejs template with comments
      }
    } catch (error) {
      console.error(error);
  ...
  app.get('/set_slot/:psy_id', async (req, res) => {
    ...
    try {
      ...
      if (psyResult.rows.length === 0) {
        ...
        res.render('set_slot', { psy }); // Render the read_more.ejs template with comments
      }
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Internal server error' });
  ...
  app.get('/take_session/:psy_id', async (req, res) => {
    ...
    try {
      ...
      if (psyResult.rows.length === 0) {
        ...
        res.render('take_session', { psychologist }); // Render the read_more.ejs template with comments
      }
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Internal server error' });
